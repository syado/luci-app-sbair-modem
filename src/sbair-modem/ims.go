...
// IMS の状態と切替。
//
// **出荷状態ではモデム側で IMS が無効。** この機体は音声サービスを持たないので
// ベンダが切っている。**SMS は IMS 経由で配送されるため、未登録だと 1 通も
// 届かない**(モデムの保存領域が 0 のまま、`AT+CNMI=1,1` にしても増えない)。
//
...
//
//	未登録 (2,0,0) から 1 → 10 秒ほどで 2,1,5 (音声 + SMS over IMS)
//	登録済み (2,1,5) で 0 → 2,1,4 になり、**登録と SMS over IMS は残った**
//
// つまり **0 にしても IMS が完全に落ちるとは限らない**。表示は ext_info の
